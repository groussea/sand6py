...
public:
	virtual void step(const Config &config, const Scalar dt,
	                  Phase& phase, Stats &stats,
	                  std::vector<RigidBody> &rigidBodies,
	                  std::vector<RBStresses> &rbStresses
	                  ) const = 0 ;

	virtual ~PhaseSolverBase() {}
} ;


class PhaseSolver : public PhaseSolverBase {
...
private:
	...
	void solveIncompressibility(const Config&c, const Scalar dt,
	                          const PhaseStepData& stepData ,
	                          std::vector< RigidBodyData >& rbData,
	                          DynVec &u, Phase &phase, Stats &stats ) const ;
