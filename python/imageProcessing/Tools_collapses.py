...
def mask_collapses(frame,OR,thresh,scale): 
...
#    frame_gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    th, BW = cv2.threshold(frame_gray,thresh*255,255,cv2.THRESH_BINARY)
    ...
    cv2.floodFill(im_floodfillL, mask, (20,0), 255)
    ...
    im_floodfillR=BW.copy()
       # Floodfill from point (0, 0)
    mask = np.zeros((h+2, w+2), np.uint8)
    cv2.floodFill(im_floodfillR, mask, (w-10,10), 255)
...
def mask_collapses2(frame,thresh): 
