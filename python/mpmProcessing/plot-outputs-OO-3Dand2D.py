...
#%%
N=7
selectedRuns,selectedDict=d6py.whereSand6OutFromParms(listNumRun,runNumber=N,frac_h=0.9)
...
runExp1=d6py.ExperimentalRun(N,mainExpFolder)
...
for ifile in range(100,105,5):
    ...
    for sR,i in zip(selectedRuns,range(len(selectedRuns))):
        sR.loadVTK(ifile)
        sR.plotContour(ax,levels=[0.5],linewidths=1,linestyles=ls[i%4],colors=[cmap(i/len(selectedRuns))])
