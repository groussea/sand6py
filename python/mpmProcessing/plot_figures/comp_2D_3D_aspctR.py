...
R_ref, selectedDict = d6py.whereSand6OutFromParms(listNumRun,  runNumber=Nrun, dimSim=3, delta_mu=0.0,delta_mu_start=0,keyWord='R_12', muRigid=0.23)

R2, selectedDict = d6py.whereSand6OutFromParms(listNumRun,  runNumber=Nrun, dimSim=3, delta_mu=0.0,delta_mu_start=0,keyWord='R_12', muRigid=0.23)
...
# for ifile in [6, 12, 3]:
    ...
    for sR, i in zip(SR, range(len(SR))):
        ...
        if sR.dimSim == 2:
            l = [r"free surf. $\mu_{"+format(sR.dimSim,'1.0f')+"D }=$"+format(sR.dConfig['mu'],'0.2f')]
        if sR.dimSim == 3:
            l = [r"free surf. $\mu_{"+format(sR.dimSim,'1.0f')+"D} (W= "+format(sR.dConfig['box'][1]*100*6/8,'0.0f')+"~\mathrm{cm})=$"+format(sR.dConfig['mu'],'0.2f')]
...
folders = ["B00_H_12_W_1_R1_a",
           ...
           "B00_H_13_W_4_R1.1_a"]
datas = []
cs= ['k', 'y', 'deeppink', 'darkred', 'grey', 'cyan']
labels=['W_1cm_R_1_a', 'W_1cm_R_1_b', 'W_4cm_R_1_a', 'W_4cm_R_1_b', 'W_1cm_R_1.1_a', 'W_4cm_R_1.1_a']
...
for f in folders[:-2]:
    ...
    line=axt.plot((data[:,5]-200-12*30)/scale,(-data[:,6]+495)/scale,'+',c=cs[i], label=labels[i], lw=0.5)
    datas.append(data)
    h1, l1 = h1 +line  , l1+  [labels[i]]
    i+=1  
    
  
fig.legend(h1  , l1 , fontsize=leg_fontsize,loc=3, framealpha=0.,edgecolor='w',facecolor='w',ncol=2,bbox_to_anchor=(0.1, 0.005, 0.4, 0.2))
...
# axs[1, 0].remove()
# axs[1,1].remove()
# axt.set_position([0.1,0.3,0.8,0.8])    
# fig.set_size_inches(4.5,3)
# ax.set_xlim(-12,25)
# for t in fig.texts:
#     t.set_visible(False)
# plt.show()
fig.savefig(driveFolder+"/programs/gitLab/dry-granular/doc/article/figures/"+fignames[Nrun]+"_1cm_mu_2D_3D_0.5.pdf", dpi=1200)
