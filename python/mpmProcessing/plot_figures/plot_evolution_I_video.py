...
cbaxes = fig.add_axes([0.38, 0.9, 0.58, 0.05])
sR.loadVTK(30)
sR.calculatePointsLayer() 
ptsI=sR.selectedInd.copy()
ind = np.random.choice(np.arange(len(sR.pointsLayer)), 10000, replace=False)
im=sR.opyfPointCloudColoredScatter(axs[0],mod='inertia',vmin=0,vmax=0.1,alpha=0.9,cmap='magma_r', rasterized=True, s=2,nvec=10000,ind=ind)
...
# for i_t in range(ideb,ideb+1):
# for i_t in range(210,211):
...
# ax.plot(times,Iqlows,'r--',label=' I quantile 40', lw=1)
    ...
    axs[1].plot([s_t, s_t],[Iqlows[i_t-ideb], Iqhighs[i_t-ideb]],'r-',lw=1.5,alpha=0.7)
    ...
    im=sR.opyfPointCloudColoredScatter(axs[0],mod='inertia',vmin=0,vmax=0.1,alpha=0.9,cmap='magma_r', rasterized=True, s=2,nvec=20000,ind=ind,calPointsLayer=False)
    sR.plotDoor(axs[0],alpha=0.8)
    ...
    ypos=0.18
    H0=11
    axs[0].set_position([0.32, ypos, 0.67, 0.55])
    axs[0].set_xlim([-22/H0,85/H0])
    axs[0].set_ylim([-1/H0,1.05])
    axs[1].set_position([0.05, ypos, 0.22, 0.75])
    ...
    axs[1].grid()
    plt.show()
    axs[0].text(24/H0,6./H0,r'$t='+format(s_t,'1.1f')+'$ s',fontsize=14,)
    ...
    fig.savefig("/media/gauthier/DataSSD/programs/gitLab/dry-granular/doc/article/video/evolution_I_"+format(i_t,'03.0f')+".png",dpi=300)
