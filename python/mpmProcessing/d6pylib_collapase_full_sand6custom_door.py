...
#driveFolder='/media/gauthier/Lacie_external/'
...
for j in range(0,8):
    ...
    #Modify the DefaultScenar file and perform a cmake
    muRigid=0.4
    
    Hmod=(sdictE['H']+0.002)/ceofCollapsevVert
    delta_mu=0.
    ts=20
    substeps=4
    sdictE['delta_mu']=delta_mu
#    runName=str('Run_'+format(j,'02.0f')+'_Door_muRigid='+str(muRigid)+'_'+sdictE['grainType']+'_Slope='+format(sdictE['Slope'],'.0f')+'deg_H='+format(sdictE['H'],'.3f')+'m_L='+format(sdictE['L'],'.3f')+'_delta_mu='+format(delta_mu,'.3f')+'_substeps_'+str(substeps))    
    runName=str('Run_'+format(j,'02.0f')+'_no_Door_start_at_0.13_s_'+sdictE['grainType']+'_Slope='+format(sdictE['Slope'],'.0f')+'deg_H='+format(sdictE['H'],'.3f')+'m_L='+format(sdictE['L'],'.3f')+'_delta_mu='+format(delta_mu,'.3f')+'_substeps_'+str(substeps))      
    ...
    d6py.modifyConfigFile(newConfigFile,newConfigFile,'mu',[sdictE['mu']+0.03])
    ...
    if j<=3:
#        d6py.modifyConfigFile(newConfigFile,newConfigFile,'scenario','collapselhedoor taudoor:0.06 veldoor:0.8 ts:'+format(ts,'1.0f'))
        d6py.modifyConfigFile(newConfigFile,newConfigFile,'scenario','collapselhedoor taudoor:0.0001 veldoor:100 ts:'+format(ts,'1.0f'))
    else:
#        d6py.modifyConfigFile(newConfigFile,newConfigFile,'scenario','collapselhedoor taudoor:0.13 veldoor:0.7 ts:'+format(ts,'1.0f'))
        d6py.modifyConfigFile(newConfigFile,newConfigFile,'scenario','collapselhedoor taudoor:0.0001 veldoor:100 ts:'+format(ts,'1.0f'))
    ...
    d6py.modifyConfigFile(newConfigFile,newConfigFile,'columnLength',[L]) #On garde la meme resolution quelque soit la longueur
    ...
    writingTracksPath=out_Opyf+listOutOpyf[j]+'/DATA_Exp_Tracks'
    ...
    writingEDProfPath=out_Opyf+listOutOpyf[j]+'/DATA_Exp_Depth'
    ...
    Ratioy=0.9 # can be modify in the Default scenarion file
...
#        plt.pause(0.01)
        ...
        Hfig=8
        Lfig=dConfig['box'][0]/dConfig['box'][1]*0.5*Hfig    
        ...
        #Determine the position of the origine by ratios for a good extent             
        ...
        fig,ax=opyf.opyffigureandaxes(extent=extentr,Hfig=4,num='a')
...
#        plt.pause(0.1)
        ...
        d6Depth.append(H)
        ifile+=1
    writingModDepthPath=out_Opyf+listOutOpyf[j]+'/DATA_Model_Sand6_Depth'
