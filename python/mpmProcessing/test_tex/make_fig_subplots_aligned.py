...
# %%
    ...
    N = 2  # lignes
    M = 3 # colonnes
    fig, axs = plt.subplots(N, M, dpi=142, figsize=(w_fig * 0.5, 2 * 0.5))
    ...
    for i in range(N):
        for j in range(M):
            ...
            axs[i, j].set_position(
                [w_s+(w_axs + w_s2) * j, (h_axs+0.05 ) * (N-i-1)+0.08, w_axs*3, h_axs])
    ...
    for ifile in [6, 12, nF]:
        ...
        for sR, i in zip(SR, range(len(SR))):
            ...
            sR.plotContour(axt, levels=[0.5], linewidths=c[i % 4], linestyles=ls[i % 4])
            sR.plotContour(ax, levels=[0.5], linewidths=c[i % 4], linestyles=ls[i % 4])
            ...
            if ifile<nF:
                for ii in [0,1]:
                    for cont in contours:
                        [line2D_vel_mod] = axs[ii, k].plot(smooth(cont[:, 0],5)*100, smooth(cont[:, 1],5)*100, linestyle='--', color='blue', linewidth=0.8, alpha=0.7, label="limit-mod")
            ...
            im = sR.opyfPointCloudColoredScatter(
                axt, nvec=6000, mute=True, vmin=0, vmax=1, s=0.8, cmap=cmap, rasterized=True, mod=mod)
    ...
    fig.savefig("/media/gauthier/Data-Gauthier/Gauthier/TAF/TAF_inria/INRIA_current_work/GitLab/dry-granular-all/dry-granular/doc/article/images/used_images/"+fignames[Nrun]+"_eps="+format(lost,'0.2f')+".pdf", dpi=150)
