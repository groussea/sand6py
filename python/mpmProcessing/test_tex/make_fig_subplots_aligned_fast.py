...
for i in range(N):
    ...
    for j in range(M):
        ...
        axs[i, j].set_position(
            [w_s+(w_axs + w_s2) * j, (h_axs+0.05 ) * (N-i-1)+0.02, w_axs*4, h_axs])
    ...
    for ifile in [6, 12, nF]:
        ...
        #     norm=(runExps[i].Ux[-1]**2+runExps[i].Uy[-1]**2)**0.5    
            ...
            for cont in contrs_vel:
                ...
                axs[i, k].plot(smooth(cont[:, 0],10)*100, smooth(cont[:, 1],10)*100, linestyle='--', color='purple', linewidth=0.8, alpha=0.7, label="Exp.")
        ...
        if ifile<nF:
            for cont in contours:
                [line2D_vel_mod] = axs[i, k].plot(smooth(cont[:, 0],5)*100, smooth(cont[:, 1],5)*100, linestyle='--', color='blue', linewidth=0.8, alpha=0.7, label="limit-mod")
