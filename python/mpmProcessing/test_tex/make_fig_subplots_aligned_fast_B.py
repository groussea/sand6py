...
N = 5  # lignes
M = 3 # colonnes
fig, axs = plt.subplots(N, M, dpi=142, figsize=(7 , 5))
...
for i in range(N):
    ...
    for j in range(M):
        ...
        axs[i, j].set_position(
            [w_s+(w_axs + w_s2) * j, (h_axs+0.04 ) * (N-i-1)+0.06, w_axs*4, h_axs])
...
for sR, i in zip(selectedRuns, range(len(selectedRuns))):
    ...
    for ifile in [6, 12, nF]:
        ...
        contrs=np.array(dictArt[fignames[i+4]]['experiment']['free_surface'][str(ifile)])
...
plt.figtext(x+X/2, y+Y+0.015, r'$t=0.6$ s',
            fontsize=9, horizontalalignment='center')
[x, y, X, Y] = axs[0, 2].get_position().bounds
plt.figtext(x+X/2, y+Y+0.015, r'$t_f$',
            fontsize=9, horizontalalignment='center')
...
plt.pause(0.1)
plt.pause(0.1)
fig.savefig("/media/gauthier/Data-Gauthier/Gauthier/TAF/TAF_inria/INRIA_current_work/GitLab/dry-granular-all/dry-granular/doc/article/images/used_images/B.pdf", dpi=150)
