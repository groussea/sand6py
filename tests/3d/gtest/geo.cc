...
TEST( geo, quad ) {
	...
	for( int k = 0 ; k < QuadPoints::NQ ; ++k ) {

		typename QuadPoints::QuadPoint qp ;
		QuadPoints::get( vx, k, qp );
		const Scalar qp_w = QuadPoints::weight( vx, k );

		f_cst  += qp_w ;
		f_lin  += qp_w * qp.prod() ;
		f_quad += qp_w * qp.prod() * qp.prod() ;
	}
